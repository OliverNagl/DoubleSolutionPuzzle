        ...
        if var > 0:  # Only consider positive literals
            ...
            if jigs is not None:
                if var in revers_jig_vars:
                    x_jig, y_jig, i, j, o = revers_jig_vars[var]
...
def solve(n,m, jigs, card=None, diff = 0,Same_pieces_k=0):
    solutions = []
    model = []
    ...
    for i in range(2):
        ...
        if i == 1 and model[0] is not None:
            dissable_solution(n, solver, model[0], edge_vars, jig_vars, jigs, pool, bound=diff, Same_pieces_k=Same_pieces_k)
...
def dissable_solution(n, solver, model, edge_vars, jig_vars, jigs, pool, bound=5, Same_pieces_k= 0):
    ...
    for (x,y,i,j,o), var in jig_vars.items():
        ...
        for rotation in range(1, 4):  # Rotate by 90, 180, 270 degrees
            rotated_y, rotated_x  = rotate_coordinates(x, y, n, rotation)
            for o in range(4):
                rotated_var = jig_vars[(x,y,rotated_y,rotated_x , o)]
                rotation_clause.append(rotated_var)

    
    rot = CardEnc.atmost(lits=rotation_clause, bound=bound,encoding=1, vpool=pool)
    solver.append_formula(rot.clauses)

    dissable_k = len(dissable_clause) - Same_pieces_k
    enc = CardEnc.atleast(lits=dissable_clause, bound=dissable_k,encoding=1, vpool=pool)
...
def rotate_coordinates(x, y, n, rotation):
    """Rotate the coordinates (x, y) on an n x n grid by 90, 180, or 270 degrees."""
    if rotation == 1:  # 90 degrees
        ...
        return x, y  # 0 degrees (no rotation)
